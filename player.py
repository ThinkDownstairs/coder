...
class Player(object):
    ...
    def render(self, screen, mouse_pos, height, axis, p_color, fps):
        if axis == "progger": # blue shape
            self.rect = pygame.Rect(mouse_pos[0], height - consts.CODER_H, consts.CODER_W, consts.CODER_H)
        if axis == "catcher": # red shape
            self.rect = pygame.Rect(0, mouse_pos[1], consts.CODER_W, consts.CODER_H)
